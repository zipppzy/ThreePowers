...
void SkillDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
    ...
    double maxXp = index.data(Qt::UserRole + 4).toDouble();  // MaxXpRole
    ...
    painter->drawText(rect.adjusted(8, 24, -8, -4), Qt::AlignLeft, QString("Lv %1").arg(level));
    ...
    double progress = maxXp > 0 ? xp / maxXp : 0;
